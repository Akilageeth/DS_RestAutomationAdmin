...
<section id="content">
	<div class="g12 nodrop">
		<h1>
			Unit Settings
			<button onclick="window.location=href='add_units.jsp#addunit'"
				style="float: right">Insert Unit</button>
		</h1>
	</div>
	<div class="g12">
		<form>
			<fieldset>
				...
				<section>
					...
					<div>
						<input type="text" id="input" name="unit_name"
							onkeyup="showHint(this.value,'unitSuggest','suggestions/unitSuggestions.jsp')">
					</div>
  ...
  %>
		...
		<form id="addunit" name="unitForm" action="actions/unit.jsp"
			method="post">
			<fieldset>
  ...
  else
  {
	%>
		...
		<form id="editunit" name="unitFormUpdate" action="actions/unit.jsp"
			method="post">
			<fieldset>
      ...
      else if(action.equals("update_success"))
		{
			%>
				...
				<section>
					<label for="input">Unit Name</label>
					<div>
						<input type="text" id="input" name="unitName"
							value="<%=request.getParameter("unitName") %>" required>
					</div>
				</section>
				<section>
					<label>Status</label>
