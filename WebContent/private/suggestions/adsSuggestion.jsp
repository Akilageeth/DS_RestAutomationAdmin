...
%>
		...
		<tr>
			...
			<td><%=var.getStatus() %></td>
			<td><a
				href="<%=constant.getAddAds() + "?id=" + var.getAdId() + "&adName=" + var.getAdName() +"&desc=" + var.getAdDescription()+"&image=" + var.getBannerName() + "&status=" + var.getStatus()+"#edit"%>">Edit</a></td>
			<td><button
					onclick="deleteRecord('<%=var.getAdId() %>','actions/<%=constant.getDelAds()%>','adSuggest');">Delete</button></td>
		</tr>
		<% 
	}
%>
