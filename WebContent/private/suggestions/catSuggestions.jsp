...
%>
		...
		<tr>
			<th><%=count++ %></th>
			<td><%=var.getCatName() %></td>
			<td><%=var.getCatStatus() %></td>
			<td><a
				href="<%=constant.getCategory() + "?id=" + var.getCatId() + "&catName=" + var.getCatName() +"&image=" + var.getCatImage() + "&status=" + var.getCatStatus()+"#editcategory"%>">Edit</a></td>
			<td><button
					onclick="deleteRecord('<%=var.getCatId() %>','actions/<%=constant.getDeleteCategory()%>','suggest');">Delete</button></td>
		</tr>
		<% 
	}
%>
