...
%>
		...
		<tr>
			<th><%=count++ %></th>
			<td><%=var.getUnitName() %></td>
			<td><%=var.getStatus() %></td>
			<td><a
				href="<%=constant.getAddUnit() + "?id=" + var.getUnitID() + "&unitName=" + var.getUnitName() + "&status=" + var.getStatus()+"#editunit"%>">Edit</a></td>
			<td><button
					onclick="deleteRecord('<%=var.getUnitID() %>','actions/<%=constant.getDeleteUnit()%>','unitSuggest');">Delete</button></td>
		</tr>
		<% 
	}
%>
