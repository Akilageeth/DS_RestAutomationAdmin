...
%>
		...
		<tr>
			<th><%=count++ %></th>
			<td><%=var.getDishName()%></td>
			<td><%=var.getDishStatus()%></td>
			<td><a
				href="<%=constant.getDish() + "?id=" + var.getDishID() + "&dishName=" + var.getDishName() + 
	...
	%>">Edit</a></td>
			...
			<td>
				<button
					onclick="deleteRecord('<%=var.getDishID() %>','actions/<%=constant.getDeleteDish()%>','dishSuggest');">Delete</button>
			</td>
		</tr>
		<% 
	}
