...
%>
		...
		<tr>
			<th><%=count++ %></th>
			<td><%=var.getIngredientName() %></td>
			<td><%=var.getIngredientStatus() %></td>
			<td><a
				href="<%=constant.getAddIngredient() + "?id=" + var.getIngredientID() + "&ingredientDishID=" + var.getIngredientDishID() +"&name=" + var.getIngredientName() + "&status=" + var.getIngredientStatus()+"#editingredients"%>">Edit</a></td>
			<td><button
					onclick="deleteRecord('<%=var.getIngredientID() %>','actions/<%=constant.getDelIngredient()%>','suggestIngredient');"">Delete</button></td>
		</tr>
		<% 
	}
%>
